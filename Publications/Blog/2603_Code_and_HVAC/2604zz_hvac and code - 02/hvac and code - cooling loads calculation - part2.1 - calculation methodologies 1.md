...
Тепловой поток, Вт, через массивную ограждающую конструкцию (наружную стену или покрытие) Q M, для данного часа суток (Z) следует определять по формуле:
$$
Q_{m, k} =  \left[ \frac{1}{R} \cdot \left( t_{out} + x_1\cdot \frac{j_{av}}{\alpha_{out}} - t_{in} \right) + \frac{\beta \cdot \alpha_{in}}{\upsilon} \cdot \left(  0.5 \cdot \theta_1 \cdot A_{air-day}+\frac{x_1}{\alpha_{out}}\cdot \theta_2 \cdot A_j   \right)  \right] \cdot A_m \tag{19}
$$
- $Q_m$ — тепловой поток через массивную ограждающую конструкцию — наружную стену или покрытие, Вт;
- $R$ — сопротивление теплопередаче массивной ограждающей конструкции (наружной стены, покрытия), $\text{м}^2 \cdot {}^\circ\text{C}/\text{Вт}$;
- $t_{out}$ — средняя температура наружного воздуха в июле, ${}^\circ\text{C}$;
- $t_{in}$ — температура воздуха в помещении, ${}^\circ\text{C}$;
- $x_1$ — коэффициент поглощения солнечной радиации поверхностью ограждающей конструкции;
- $j_{av}$ — среднесуточное значение поверхностной плотности теплового потока суммарной солнечной радиации (прямой и рассеянной), $\text{Вт}/\text{м}^2$. Для горизонтальной поверхности принимается по таблице 7, для вертикальной поверхности — по таблице 8;
- $\alpha_{out}$ — коэффициент теплоотдачи наружной поверхности ограждающей конструкции, $\text{Вт}/(\text{м}^2 \cdot {}^\circ\text{C})$;
- $\alpha_{in}$ — коэффициент теплоотдачи внутренней поверхности ограждающей конструкции, $\text{Вт}/(\text{м}^2 \cdot {}^\circ\text{C})$;
- $\beta$ — коэффициент, принимаемый равным $1$ при отсутствии вентилируемой воздушной прослойки в ограждающей конструкции (покрытии) и равным $0{,}6$ для остальных ограждающих конструкций;
- $\upsilon$ — величина затухания амплитуды колебаний температуры наружного воздуха в ограждающей конструкции;
- $\theta_1$, $\theta_2$ — коэффициенты, принимаемые по таблице 6 для каждого часа суток с учётом запаздывания температурных колебаний в ограждающей конструкции;
- $A_{air-day}$ — максимальная суточная амплитуда температуры наружного воздуха в июле, ${}^\circ\text{C}$;
- $A_j$ — амплитуда суточных колебаний суммарной солнечной радиации (прямой и рассеянной), $\text{Вт}/\text{м}^2$;
- $A_m$ — площадь массивной ограждающей конструкции (наружной стены, покрытия), $\text{м}^2$.
